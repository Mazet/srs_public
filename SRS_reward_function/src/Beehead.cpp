// Beehead.cpp : Defines the entry point for the console application.
//
...
/*    		 if (fit[0] > ans)		//ans is threshold for accepted result
    			{
    			  //Result<<iter+1<<"\t" <<(iter+1)*n<<"\t" <<"\t1\t"<<n<<"\t"<<fit[0]<<endl;
				Result<<iter+1<<"," <<(iter+1)*n<<"," <<"1,"<<n<<","<<fit[0]<<endl;
    			  break;
    			  }*/
	  ...
	  //if (fit[0] < ans) fail++;	//fail to reach the threshold
